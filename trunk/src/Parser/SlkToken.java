...
//	***************************************************************************//
    ...
    public short get() {
        ...
        TipoToken tipo = _token.getTipoToken();


        if (tipo.compareTo(TipoToken.valueOf("IDENTIFICADOR"))==0){
            Nodo nodo = new Nodo();
            nodo.concatRight(tipo);
            nodo.setValor(_token.getAtributo());
            PilaNodos.getInstancia().push(nodo);
        }
        //no entra nunca porque la palabra INTEGER es una palabra reservada
        if(tipo.compareTo(TipoToken.valueOf("PALABRA_RESERVADA"))==0 && _token.getAtributo().equals("INTEGER") ){
            Nodo nodo = new Nodo();
            nodo.concatRight(tipo);
            nodo.setValor(_token.getAtributo());
            nodo.concatRight(tipo);
            System.out.println("entra ya");
            PilaNodos.getInstancia().push(nodo);
        }
...
//	***************************************************************************//
    ...
    public Character asciiToChar(String lexema) {
        ...
        if ((num > 127) || (num < 0)) {
            ...
            String Cha = new Character((char) (Integer.parseInt(cad))).toString();
